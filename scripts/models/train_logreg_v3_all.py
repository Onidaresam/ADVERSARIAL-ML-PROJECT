...
- Trains/predicts 384 Logistic Regression classifiers (one per bin)
...
parser.add_argument("--sensory_mask_scope",
                    choices=["entry", "row"],
                    default="entry",
                    help="Sensory mask: 'entry' = N×384, 'row' = N,")
...
OUT_FOLDER        = f"{SEG_FOLDER}/models_reducedscope/logreg_v3"
TEST_SPLIT_FOLDER = f"{SEG_FOLDER}/models_reducedscope/testsplits_logreg_v3"
DETAIL_FOLDER     = f"{SEG_FOLDER}/models_reducedscope/logreg_v3_details"
...
print(f"\n[train_logreg_v3_all] Segment {SEGMENT_ID}")
...
if __name__ == "__main__":
    ...
    # 0) Load full clean + poisoned datasets
    ...
    BASELINE_MODEL_PATH = f"{OUT_FOLDER}/seg{SEGMENT_ID}_lr_v3_clean_models.pkl"
    BASELINE_PRED_PATH  = f"{TEST_SPLIT_FOLDER}/seg{SEGMENT_ID}_lr_v3_clean_pred.npy"
    ROBUST_METRICS_PATH = f"{OUT_FOLDER}/seg{SEGMENT_ID}_lr_v3_robust_{ATTACK}_{PCT}_metrics.csv"
    ADV_MODEL_PATH      = f"{OUT_FOLDER}/seg{SEGMENT_ID}_lr_v3_advtrain_{ATTACK}_{PCT}_models.pkl"
    ...
    # BASELINE
    if args.mode == "full":
        print("\n[BASELINE] Training 384 LR models on CLEAN data (row-matched)...")
        t0 = time.time()
    ...
    if args.mode in ["full", "adv_only"]:
        ...
        pred_adv = predict_lr_models(adv_models, Xp_test)
        np.save(
    ...
    print(f"\n[TOTAL] train_logreg_v3_all run completed in {total_runtime:.2f} seconds.\n")
