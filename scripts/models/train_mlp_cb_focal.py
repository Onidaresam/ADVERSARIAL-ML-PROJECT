...
OUT_FOLDER = f"{SEG_FOLDER}/models_reducedscope/mlp_cb_focal"
TEST_SPLIT_FOLDER = f"{SEG_FOLDER}/models_reducedscope/testsplits_cb_focal"
BIN_INFO_FOLDER = f"{SEG_FOLDER}/models_reducedscope/bin_info"
DETAIL_FOLDER = f"{SEG_FOLDER}/models_reducedscope/mlp_cb_focal_details"
...
class CBFocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        pt = probs * targets + (1 - probs) * (1 - targets)  # (B, C)

        focal_factor = (1 - pt) ** self.gamma  # (B, C)

        # apply class weights
        cw = self.class_weights.unsqueeze(0)  # (1, C)
        loss = focal_factor * bce * cw  # (B, C)
...
def compute_sample_weights(y_train):
    """
    Simple heuristic:
    - samples with more positives get higher weight
    - ensures positives are oversampled
    """
    pos_counts_per_sample = y_train.sum(axis=1)  # (N,)
    # base weight 1.0, add 2.0 for any positive, more if multiple
...
if __name__ == "__main__":
    print(f"\nRunning CB-Focal MLP pipeline for segment {SEGMENT_ID}")
    ...
    # Filter bins by positives on full clean labels
    ...
    # Class-balanced focal loss weights (per class)
    ...
    # BASELINE
    # ========================================================
    print("\n[BASELINE] Training CB-Focal MLP on CLEAN data...")
    ...
    # ROBUSTNESS
    # ========================================================
    print(f"\n[ROBUSTNESS] Evaluating clean-trained CB-Focal MLP → {ATTACK} {PCT}%")

    Xp, yp_full, yp_var = load_poisoned_features_labels(ATTACK, PCT)
    yp_var = yp_var[:, keep_mask]  # same kept bins
    ...
    compute_per_bin_metrics(yp_test_var, preds_var_robust, kept_var_bins, phase_name="robust", attack=ATTACK, pct=PCT)
    ...
    # ========================================================
    print(f"\n[ADV TRAIN] Training CB-Focal MLP on {ATTACK} {PCT}% poisoned data...")
    ...
    print("CB-Focal MLP pipeline run completed.")
