...
if __name__ == "__main__":
    ...
    # CHECKPOINT PATHS
    # ========================================================
    BASELINE_PATH = f"{OUT_FOLDER}/seg{SEGMENT_ID}_xgb_clean_models.pkl"
    ROBUST_PATH   = f"{OUT_FOLDER}/seg{SEGMENT_ID}_xgb_robust_{ATTACK}_{PCT}_metrics.csv"
    ADV_PATH      = f"{OUT_FOLDER}/seg{SEGMENT_ID}_xgb_advtrain_{ATTACK}_{PCT}_models.pkl"
    ...
    if args.mode == "full":
        if not os.path.exists(BASELINE_PATH):
            print("\n[BASELINE] No baseline found → training 384 XGBoost models...")
            ...
            print(f"[BASELINE] Runtime: {baseline_end - baseline_start:.2f} seconds")

        else:
            print("\n[BASELINE] Found existing baseline → loading.")
            base_models = joblib.load(BASELINE_PATH)
            pred_clean = np.load(f"{TEST_SPLIT_FOLDER}/seg{SEGMENT_ID}_xgb_clean_pred.npy")
    ...
    if args.mode == "full":
        if not os.path.exists(ROBUST_PATH):
            print(f"\n[ROBUSTNESS] No robustness results → evaluating clean-trained XGBoost → {ATTACK} {PCT}%")
            ...
            np.save(f"{TEST_SPLIT_FOLDER}/seg{SEGMENT_ID}_xgb_robust_{ATTACK}_{PCT}_pred.npy", pred_robust)
            ...
            df_main_robust.to_csv(ROBUST_PATH, index=False)

            robust_end = time.time()
            print(f"[ROBUSTNESS] Runtime: {robust_end - robust_start:.2f} seconds")
    ...
    if args.mode == "full":
        if not os.path.exists(ADV_PATH):
            print(f"\n[ADV TRAIN] No adversarial model → training XGBoost on {ATTACK} {PCT}% poisoned data...")
            ...
            adv_end = time.time()
            print(f"[ADV TRAIN] Runtime: {adv_end - adv_start:.2f} seconds")
