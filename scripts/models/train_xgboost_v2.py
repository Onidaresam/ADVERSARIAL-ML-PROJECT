...
import torch  # still imported, but not used for device detection
...
parser.add_argument(
    ...
    help="full = baseline + robustness + adversarial, adv_only = only adversarial training (baseline still trained)"
)
...
def get_xgb_device_params():
    ...
    based on actual XGBoost GPU capability.
    """
    try:
        X_test = np.zeros((10, 1), dtype=np.float32)
        y_test = np.zeros(10, dtype=np.int32)
...
if __name__ == "__main__":
    print(f"\n[train_xgboost_v2] Running XGBoost v2 for segment {SEGMENT_ID}")
    ...
    # -----------------------------
    # PATHS (NAMES MATCH EXISTING CONVENTION)
    # -----------------------------
    ...
    # ========================================================
    # BASELINE (clean → clean) — ALWAYS TRAIN
    # ========================================================
    ...
    # ========================================================
    # LOAD POISONED DATA ONCE (USED BY ROBUST + ADV)
    # ========================================================
    Xp, yp = load_poisoned(ATTACK, PCT)

    # ========================================================
    # ROBUSTNESS (clean-trained → poisoned test) — FULL MODE ONLY
    # ========================================================
    if args.mode == "full":
    ...
    # ========================================================
    # ADVERSARIAL TRAINING (poisoned → poisoned) — ALWAYS TRAIN
    # ========================================================
    ...
    print(f"\n[TOTAL] train_xgboost_v2 run completed in {total_runtime:.2f} seconds.\n")
